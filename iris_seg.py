...
def getIris(frame):
	...
	for circle in circles:
		...
		#cv.ShowImage("resImg1", resImg)
		#cropImg = cv.CreateImage((w,h), 8, 3)
		#print(cropImg)
		#cv.ShowImage("cropImg", cropImg)
		#cv.ShowImage("resImg", resImg)
...
count=1
while count<21:
	eyesList = os.listdir('CASIA-Iris-Interval/'+str(count).zfill(3)+'/R')
	...
	if(k==0):
		...
		while(w!=k):
			eye = getNewEye(eyesList)
			frame = cv.LoadImage('CASIA-Iris-Interval/'+str(count).zfill(3)+'/R/'+eye)
	#frame = cv.LoadImage("images/eyes/"+eye)
			...
			cp.crop(normImg,count,w)
	#cv.SaveImage('pic{:>05}.jpg'.format(count), normImg)
			key = cv.WaitKey(3000)
